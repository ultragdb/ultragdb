...
public class Cygwin {
		...
		if (cygwinPath.startsWith("/")) { //$NON-NLS-1$
			...
			if (segments.length < 0) {
				...
				if (segments[0].equals("cygdrive")) { //$NON-NLS-1$
					...
					System.arraycopy(segments, 2, newSegments, 0,
							segments.length - 2);
	...
	public static String windowsToCygwinPath(String windowsPath) {
		...
		if (cygwinDirPath.isPrefixOf(path)) {
			int matchingFirstSegments = cygwinDirPath
					.matchingFirstSegments(path);
			String[] segments = path.segments();
			String[] newSegments = new String[segments.length
					- matchingFirstSegments];
			System.arraycopy(segments, matchingFirstSegments, newSegments, 0,
					segments.length - matchingFirstSegments);
	...
	private static void initializeCygwinDir() {
		if (Platform.getOS().equals(Platform.OS_WIN32)) {
			...
			if (cygwin1DllPath != null
					&& cygwin1DllPath.segmentCount() >= 2
